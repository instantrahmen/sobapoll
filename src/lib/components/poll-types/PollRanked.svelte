<script lang="ts">
  ...
  import { } from 'svelte-dnd-action';
  import DnDZone from '../DnDZone.svelte';
...
<Card.Root class={cn('rounded-xl my-4', className)} {...restProps}>
  ...
  <Card.Content class="mx-auto max-w-md px-4">
    ...
    <div class="mt-4 grid gap-2">
      {#if !poll.selected }
        <DnDZone items={poll.options}>
          {#snippet renderItem(item)}
            {@const id = item.id}
            {@render pollOptions({opt: item})}
          {/snippet}
        </DnDZone>
      {:else}
...
{#snippet pollVotes({option}: {option: PollWithPercentage})}
  ...
  <Progress
    ...
    indicatorClass={cn(poll.selected === id ? 'bg-secondary' : 'bg-primary/20')}
