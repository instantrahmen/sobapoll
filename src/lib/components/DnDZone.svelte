...
<script lang="ts" generics="T extends DnDZoneItem">
  ...
  type Children = Snippet<[items: T[], dragHandle: DragHandleAction]>;
  ...
  $effect(() => {
    if (children && renderItem) {
      console.error('Cannot specify both `renderItem` and `children`. Only one will be rendered');
    } else if (!children && !renderItem) {
      console.error('Must specify either `renderItem` or `children`. No item will be rendered');
    }
  })
</script>
...
<div
  ...
  onconsider={useDragHandle ? handleConsiderWithDragHandle : handleSort}
  onfinalize={useDragHandle ? handleFinalizeWithDragHandle : handleDrop}
>
  {#if children}
    {@render children(items, dragHandle)}
  {:else if renderItem}
