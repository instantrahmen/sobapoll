import type { Vote, Tally, STVRound, STVOptions, STVResults } from './model';
 ...
 */
export function getWinQuota(
  validVotesCast: number,
  seatsToFill: number,
  fractionalVotesAllowed: boolean
): number {
...
export function countVotesForCandidate(
  candidates: string[],
  votes: Vote[],
  onlyVotesOf?: string,
  skipOver?: string[]
...
export function writeInVoteWeight(
  weight: number,
  onlyVotesOf: string,
  votes: Vote[],
  skipOver?: string[]
...
export function stvRound(
  roundNumber: number,
  lastRound: STVRound | undefined,
  opts: {
    winQuota: number;
    candidates: string[];
    votes: Vote[];
  ...
  const tally = countVotesForCandidate(opts.candidates, opts.votes, undefined, [
    ...lastEliminations,
    ...lastWinners,
  ]);
  ...
  if (frontrunner.votesCount >= winQuota) {
    ...
    log(
      `✅ ${winner} ELECTED! Candidate passed quota of ${winQuota} votes with ${frontrunner.votesCount} votes.`
    );
    log(
      `♻️ Amount of surplus votes: ${surplusVotes} out of ${frontrunner.votesCount} => ${weight * 100}% of remaining ballot efectivity`
    );
    ...
    log(
      `${frontrunner.candidate} as a LAST MAN STANDING ELECTED! (# of votes ${frontrunner.votesCount})`
    );
    winner = frontrunner.candidate;
  } else {
...
export function stv(opts: STVOptions): STVResults {
  ...
  const weightedVoteCount = opts.votes
  ...
  for (let roundNumber = 1; roundNumber < 20; roundNumber++) {
    lastRound = stvRound(roundNumber, lastRound, {
      ...opts,
      winQuota: winQuota,
      votes: opts.votes,
      report: log,
